...
Game::Game() {
	...
	/////////////////
	sf::Texture texture2;
	texture2.loadFromFile("coin.png");
	Coin *coin = new Coin(texture2);
	///////////////////
	...
	while (window.isOpen())
	{
		...
		////// coin collisoin detection
		sf::FloatRect shape1 = mario->getGlobalBounds();
		sf::FloatRect shape2 = coin->getGlobalBounds();

		

		if (mario->getPosition().x < coin->getPosition().x + shape2.width &&
			mario->getPosition().x + shape1.width > coin->getPosition().x &&
			mario->getPosition().y < coin->getPosition().y + shape2.height &&
			shape1.height + mario->getPosition().y > coin->getPosition().y)
		{
			GameMusic.coinPlay();
			coin->newCoin();
		}
